...
class Ant17(Ant):
    ...
    def walk(self):
        ...
        if(limit):
            ...
            path = Path(self.x, self.y, self.dir, self.heatloss)
...
class Univers17(Univers):
    ...
    def displayPath(self, cv2, img, cCaseWidth, cCaseHeight,stop,ant):
        self.displayU( cv2, img, cCaseWidth, cCaseHeight,False)
        for w in ant.aPath:
            dx = w.x * cCaseWidth
            dy = w.y * cCaseHeight
            cv2.rectangle(self.img, (dx, dy), (dx + cCaseWidth, dy + cCaseHeight), (0xD9, 0xD9, 0xD9), -1)
            cv2.imshow('img', self.img)
            print(w.x,w.y,w.heatloss)
            key = cv2.waitKey(0) & 0x0FF
        ...
        while True:
            ...
            if(ret==True):
                ...
                if(split):
                    ...
                    if(l=="ok"):
                        self.displayPath(cv2, self.img, cCaseWidth, cCaseHeight, True,ant)
                        result=r
                if(debug>eNonelvl):
                    #self.print()
                    nb+=1
...
class Case17(Case):
    ...
    def display(self, cv2, img, cCaseWidth, cCaseHeight):
        ...
        org = (dx + 1*int(cCaseWidth / 3), dy + 2*int(cCaseHeight / 3))

        orgN = (dx + 1*int(cCaseWidth / 3), dy + 1*int(cCaseHeight / 3))
        orgS = (dx + 1*int(cCaseWidth / 3), dy + 0*int(cCaseHeight / 3))
        orgE = (dx + 2*int(cCaseWidth / 3), dy + int(cCaseHeight / 2))
        orgW = (dx + 0*int(cCaseWidth / 3), dy + int(cCaseHeight / 2))

        image = cv2.putText(img, str(self.heatlossN), orgN, font,
                             fontScale, color, thickness, cv2.LINE_AA)

        image = cv2.putText(img, str(self.heatlossS), orgS, font,
                             fontScale, color, thickness, cv2.LINE_AA)
        image = cv2.putText(img, str(self.heatlossE), orgE, font,
                             fontScale, color, thickness, cv2.LINE_AA)
        image = cv2.putText(img, str(self.heatlossW), orgW, font,
                             fontScale, color, thickness, cv2.LINE_AA)
